...
### Configuração automática (opcional, recomendada no GitHub Pages)

Para evitar depender do painel admin no primeiro acesso, você pode definir a URL do backend direto no HTML:

```html
<meta name="bncc-backend-url" content="https://SEU-BACKEND.up.railway.app">
```

Ou antes do script principal:

```html
<script>window.BNCC_BACKEND_URL = 'https://SEU-BACKEND.up.railway.app';</script>
```

O frontend passa a detectar automaticamente essa URL.
